...
for A in range(50,51,1):
	for Threshold in range(950,1000,50):
		...
		os.system('mkdir noPABO_Many')
		os.system('scavetool scalar -p \'module(*) AND ("dropPk:count")\' -O noPABO_Many/dropCount.csv -F csv General-0.sca')
		os.system('scavetool scalar -p \'module(*.host*.*.queue.dataQueue) AND ("rcvdPk:count")\' -O noPABO_Many/rcvCount.csv -F csv General-0.sca')
		os.system('scavetool vector -p \'module(*) AND ("numRTOs")\' -O noPABO_Many/numRTOs.csv -F csv General-0.vec')
		os.system('scavetool vector -p \'module(*) AND ("num FastRetransmission/Recovery")\' -O noPABO_Many/numFastRetransmit.csv -F csv General-0.vec')
		os.system('scavetool vector -p \'module(FattreewithPABO.host9.tcp) AND ("SHI: reordering density")\' -O noPABO_Many/RDOutsplit.csv -F splitcsv General-0.vec')
		os.system('scavetool vector -p \'module(FattreewithPABO.host9.tcp) AND ("SHI: packet delay")\' -O noPABO_Many/PkDelayOutsplit.csv -F splitcsv General-0.vec')
		os.system('scavetool scalar -p \'module(FattreewithPABO.host9.*) AND ("endToEndDelay:histogram:mean")\' -O noPABO_Many/endToEndDelay.csv -F csv General-0.sca')
		os.system('scavetool vector -p \'module(FattreewithPABO.host10.tcp) AND ("SHI: reordering density")\' -O noPABO_Many/RDOutsplit2.csv -F splitcsv General-0.vec')
		os.system('scavetool vector -p \'module(FattreewithPABO.host10.tcp) AND ("SHI: packet delay")\' -O noPABO_Many/PkDelayOutsplit2.csv -F splitcsv General-0.vec')
		os.system('scavetool scalar -p \'module(FattreewithPABO.host10.*) AND ("endToEndDelay:histogram:mean")\' -O noPABO_Many/endToEndDelay2.csv -F csv General-0.sca')
		
		###### please make sure the data files and matlab code file are in '/Users/shixiang/Documents/omnetpp-4.6/samples/FattreewithPABO/simulations/results/noPABO_Many'
		###### split the dumped outsplit csvfiles into several files : 1_xxx.csv , 2_xxx.csv , 3_xxx.csv ...
		os.chdir('/Users/shixiang/Documents/omnetpp-4.6/samples/FattreewithPABO/simulations/results/noPABO_Many')	
